...
def path_opti_stats(maze_map, prob_map):
    """
    Function to compute caracteristics of the maze map related to the optimal
    path.
    ...
    pr_gc_op : float
        Proportion of the ground cells used by the optimal path.
    dsb[1:] : numpy.ndarray
        Distribution of the number of connection of the cell used by the
        optimal path (for 2, 3 and 4 connections).
    num_turn : int
    ...
    # proportion of the ground cells used by the optimal path
    gc_op = path[1::2] # [1::2] : to get only ground cells
    n_gcells = ((len(maze_map)-1)/2)**2
    pr_gc_op = len(gc_op)/n_gcells

    # distribution of the number of connection of the cell used by the
    # optimal path {2, 3, 4}
    kern = gc_op + np.array([[[-1, 0]], [[0, 1]], [[1, 0]], [[0, -1]]])
    sums_k = np.sum(maze_map[kern[:, :, 0], kern[:, :, 1]] == 0, axis=0)
    uniqs, counts = np.unique(sums_k, return_counts=True)
    dsb = np.zeros(4)
    dsb[uniqs-1] = counts/np.sum(counts)
    ...
    return max_grad, len_p, pr_gc_op, dsb[1:], num_turn, max_dx, max_dy, prob_map
...
def caracterisation_orient(maze_map, ground_nodes):
    ...
    ----------
    maze_map : numpy.ndarray
        Map of the maze with -1 as walls and 0 as grounds.
    ground_nodes : numpy.ndarray
    ...
    distribution = np.zeros(4)
    values, counts = np.unique(np.argwhere(neighbors)[:, 0], return_counts=True)
    distribution[values-1] = counts/np.sum(counts)
    return distribution
